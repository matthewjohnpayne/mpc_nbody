...
@pytest.mark.parametrize('input_arguments', [('30101.eq0_postfit', 'eq'),
                                             ('30102.eq0_postfit', 'eq')])
@pytest.mark.parametrize('test_result', ['holman_ic_30101', 'holman_ic_30102'])
def test_instantiation_with_data(input_arguments, test_result):
    '''
    Test that instantiation with data functions (essentially test everything).
    '''
    data_file, filetype = input_arguments
    parse_input.ParseElements(os.path.join(DATA_DIR, data_file), filetype)
    assert cmp('./holman_ic', os.path.join(DATA_DIR, test_result))
